...
文章在列表頁的順序，靠一個叫 `order-數字` 的 tag 控制：

- 想排第 1 的文章 → 加 tag `order-1`
- 想排第 2 的 → 加 tag `order-2`
- 以此類推（`order-3`、`order-4`…）

規則：
- **數字小的排前面**（order-1 在最上）
- 這個 tag 跟 `writing` / `teaching` **並存**，一篇文章同時掛 `writing` + `order-1` 沒問題
- **沒掛 `order-` tag 的文章**，會排在所有有掛的後面，照發布日期（新→舊）
- 要換順序 → 在 Ghost 改那個數字就好，存檔等 1–3 分鐘自動生效
- 數字可以跳號（order-1、order-5、order-10 也行，照大小排），方便中間插文章

> tag 名稱要剛好是 `order-1` 這種格式（小寫 order、連字號、數字）。打成 `Order 1`、`順序1` 不會生效。
>
> 這個 tag 只有後台和系統看得到，**arxi.tw 上完全不會顯示**，讀者看不到。
